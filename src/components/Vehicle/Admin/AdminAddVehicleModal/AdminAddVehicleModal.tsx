...
import "./AdminAddVehicleModal.css";
...
const AdminAddVehicleModal = observer(
    ...
    const onValidSubmit = async (data: { vehicles: VehiclesFormData }) => {
      try {
        const transformedData = data.vehicles.map((vehicle) => ({
          client_id: Number(vehicle.client),
          brand: vehicle.brand,
          model: vehicle.model,
          v_type_id: Number(vehicle.type),
          vin: vehicle.vin,
        ...
        if (axiosError.response?.status === 400) {
          ...
          if (errors?.vin) {
            const firstVehicleIndex = 0;
            setError(`vehicles.${firstVehicleIndex}.vin`, {
              type: "manual",
              message: t("vehicleModal.errors.invalidVin"),
            });

            setTabIndex(firstVehicleIndex);
            setFocus(`vehicles.${firstVehicleIndex}.vin`);
          }
        }
      }
    };
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Select
                          ...
                          error={errors.vehicles?.[i]?.client}
                          placeholder={
                            <>
                              {t("vehicleModal.ui.client")}{" "}
